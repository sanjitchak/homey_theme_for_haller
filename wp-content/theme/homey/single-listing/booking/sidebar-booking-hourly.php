...
for ($halfhour = $start_hour; $halfhour <= $end_hour; $halfhour = $halfhour+30*60) {
   ...
   if ( str_contains(strval(date('H:i',$halfhour)), '17') ||  str_contains(strval(date('H:i',$halfhour)), '16:30') ) { 
		$start_hours_list .= '<option   disabled '.selected($pre_start_hour, date('H:i',$halfhour), false).' value="'.date('H:i',$halfhour).'">'.date(homey_time_format(),$halfhour).'</option>';
		$end_hours_list .= '<option  disabled '.selected($pre_end_hour, date('H:i',$halfhour), false).' value="'.date('H:i',$halfhour).'"> '.date(homey_time_format(),$halfhour).' </option>';
	...
	<div class="block">
		<div class="sidebar-booking-module-header">
			<div class="block-body-sidebar">
					...
					<?php 	
					echo homey_formatted_price($min_hours*$listing_price, false, true); ?><sub><?php echo esc_attr($price_separator);
					 
					?><?php echo $min_hours." ".homey_get_price_label();?></sub>
