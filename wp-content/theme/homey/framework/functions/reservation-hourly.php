...
if(!function_exists('check_hourly_booking_availability')) {
    function check_hourly_booking_availability($check_in_date, $check_in_hour, $check_out_hour, $start_hour, $end_hour, $listing_id, $guests) {
        ...
        if($hours_count < $min_book_hours) {
            $return_array['success'] = false;
            $return_array['message'] = $local['min_book_hours_error'].' '.$min_book_hours.'. You have set '.$hours_count.' hours.';
            return $return_array;
        }
        else if($hours_count > $min_book_hours) 
        {
            $return_array['success'] = false;
            $return_array['message'] = $local['max_book_hours_error'].' '.$min_book_hours.'. You have set '.$hours_count.' hours.'.' <br>'.'<p style="background-color: #31ed5e;text-align: center;color: white;padding: 10px;width: 14vw;margin: auto;"><a href="#" style="
            color: white;
            padding: 17px;
        ">Book A Call To Extend</a></p>';
            return $return_array;
        }
    ...
    function check_booking_availability_on_hour_change() {
        ...
        if($hours_count < $min_book_hours) {
            echo json_encode(
                array(
                    'success' => false,
                    'message' => $local['min_book_hours_error'].' '.$min_book_hours.'. You have set '.$hours_count.' hours.'
                )
            );
            wp_die();
        }
        else if($hours_count > $min_book_hours) 
        {
            echo json_encode(
                array(
                    'success' => false,
                    'message' => $local['max_book_hours_error'].' '.$min_book_hours.'. You have set '.$hours_count.' hours.'.' <br>'.'<p style="background-color: #31ed5e;text-align: center;color: white;padding: 10px;width: 14vw;margin: auto;"><a href="#" style="
                    color: white;
                    padding: 17px;
                ">Book A Call To Extend</a></p>'
                )
